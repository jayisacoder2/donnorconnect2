...
DonorConnect – Full System Description for Wireframes & Diagrams
System Overview

DonorConnect is a web-based donor CRM designed for small to mid-sized nonprofits. Its purpose is to centralize donor data, track donations and campaigns, and ensure consistent donor follow-ups. The system prioritizes donor retention, visibility, and operational clarity rather than payment processing.
...
These files control how the system runs and behaves:

next.config.js
Controls Next.js behavior, routing, environment variables, and build output.

tailwind.config.js
Defines the design system including colors, spacing, typography, and UI utilities.

prisma.config.js
Configures Prisma client behavior and database connections.

vitest.config.js
Defines how unit and integration tests are executed.

Database Layer

The database represents the nonprofit domain.

Core Data Models

Defined in prisma/schema.prisma:

Organization – isolates data per nonprofit

User – staff members with roles

Donor – individual or organization giving money

Donation – a gift made by a donor

Campaign – fundraising initiative

Task / Follow-up – reminders and actions

Workflow – automated sequences (thank-yous, reminders)

Supporting Files

prisma/seed.js creates realistic test data (donors, donations, campaigns).

prisma/migrations/ tracks schema changes over time.

Application Structure (Next.js App Router)
Public Authentication Pages (auth)
...
Donor list with search and filters

Quick info (last gift, total giving, status)
...
Filters by date, campaign, donor
...
Donations linked to campaign

Segments

Group donors by behavior

Used for targeting and workflows

Workflows

Automation builder

Triggers (new donation, lapsed donor)

Actions (send thank-you, create task)
...
Handles sessions and access control
...
Mock handlers simulate API responses

Unit tests verify helpers and logic
...
DonorConnect is a donor management CRM that helps nonprofits track donors, manage campaigns, log donations, and automate follow-ups to improve donor retention.